...
export function createProfileScreen({
  ...
  function renderProfile(user = {}) {
    if (dom.profile.name) {
      dom.profile.name.textContent = user.name || "Пользователь";
    }
    if (dom.profile.weight) {
      dom.profile.weight.textContent = user.weight ? `${user.weight} кг` : "—";
    }
    if (dom.profile.height) {
      dom.profile.height.textContent = user.height ? `${user.height} см` : "—";
    }
    if (dom.profile.experience) {
      dom.profile.experience.textContent = user.experience || "—";
    }
    if (dom.profile.workouts) {
      dom.profile.workouts.textContent = String(user.workout_days ?? 0);
    }
  }

  function updateHistoryManageControls() {
    if (!dom.profile.historyManageToggle || !dom.profile.historyBulkActions) {
      return;
    }

    dom.profile.historyManageToggle.classList.toggle("is-text-mode", state.historyEditMode);
    dom.profile.historyManageToggle.setAttribute(
      "aria-label",
      state.historyEditMode
        ? "Завершить редактирование истории тренировок"
        : "Изменить историю тренировок"
    );
    dom.profile.historyManageToggle.innerHTML = state.historyEditMode ? "Готово" : "<i class='bx bx-cog'></i>";
    dom.profile.historyBulkActions.hidden = !state.historyEditMode;

    if (dom.profile.historyDeleteSelectedButton) {
      dom.profile.historyDeleteSelectedButton.disabled = state.selectedWorkoutSessions.size === 0;
    }
  }
  ...
  function renderHistory(history = [], options = {}) {
    ...
    if (!dom.profile.historyList) {
      return;
    }

    dom.profile.historyList.innerHTML = "";
    updateHistoryManageControls();
    ...
    if (!history.length) {
      ...
      updateHistoryManageControls();
      dom.profile.historyList.innerHTML = emptyCard("Пока нет тренировок.");
      return;
    }
    ...
    history.forEach((day, index) => {
      ...
      dom.profile.historyList.insertAdjacentHTML(
        "beforeend",
        `
    ...
    dom.profile.historyList.querySelectorAll(".history-edit-btn").forEach((button) => {
    ...
    dom.profile.historyList.querySelectorAll(".history-card[data-session-key]").forEach((card) => {
    ...
    if (animate) {
      animateCollection(dom.profile.historyList, ".history-card");
      animateExerciseRows(dom.profile.historyList);
    }
  }
  ...
  function bindEvents() {
    dom.profile.openProfileOverlayButton?.addEventListener("click", openProfileOverlay);
    dom.profile.historyManageToggle?.addEventListener("click", toggleHistoryManageMode);
    dom.profile.historyManageCancelButton?.addEventListener("click", () => {
      disableHistoryManageMode();
    });
    dom.profile.historyDeleteSelectedButton?.addEventListener("click", () => {
      void deleteSelectedHistoryWorkouts();
    });
    dom.profile.historyDeleteAllButton?.addEventListener("click", () => {
